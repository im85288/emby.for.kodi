...
class LibraryWindow(kodigui.MultiWindow, windowutils.UtilMixin):
    ...
    def sortButtonClicked(self):
        desc = 'script.emby/indicators/arrow-down.png'
        asc = 'script.emby/indicators/arrow-up.png'
    ...
    def subOptionCallback(self, option):
        check = 'script.emby/home/device/check.png'
        options = None
        subKey = None
        if self.filter:
    ...
    def filter1ButtonClicked(self):
        ...
        if self.filter:
            options.append({'type': 'clear_filter', 'display': T(32376, 'CLEAR FILTER').upper(), 'indicator': 'script.emby/indicators/remove.png'})
    ...
    def fillShows(self):
        ...
        idx = 0
        fallback = 'script.emby/thumb_fallbacks/{0}.png'.format(TYPE_KEYS.get(self.section.type, TYPE_KEYS['movie'])['fallback'])
