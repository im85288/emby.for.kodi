...
class VideoNodes(object):
    ...
    def commonRoot(self, order, label, tagname="", roottype=1):
        ...
        etree.SubElement(root, 'icon').text = "special://home/addons/emby.for.kodi/icon.png"
    ...
    def viewNode(self, indexnumber, tagname, mediatype, viewtype, viewid, delete=False):
        ...
        for node in nodes:
            ...
            if (mediatype == "homevideos" or mediatype == "photos") and nodetype == "all":
                # Custom query
                path = ("plugin://emby.for.kodi/?id=%s&mode=browsecontent&type=%s"
                        % (tagname, mediatype))
            elif (mediatype == "homevideos" or mediatype == "photos"):
                # Custom query
                path = ("plugin://emby.for.kodi/?id=%s&mode=browsecontent&type=%s&folderid=%s"
    ...
    def singleNode(self, indexnumber, tagname, mediatype, itemtype):
        ...
        if itemtype == "channels":
            ...
            etree.SubElement(root, 'path').text = "plugin://emby.for.kodi/?id=0&mode=channels"
        elif itemtype == "favourites" and mediatype == "episodes":
            root = self.commonRoot(order=1, label=label, tagname=tagname, roottype=2)
            etree.SubElement(root, 'path').text = "plugin://emby.for.kodi/?id=%s&mode=browsecontent&type=%s&folderid=favepisodes" %(tagname, mediatype)
        else:
